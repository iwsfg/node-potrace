...
Posterizer.prototype = {
  _getSamplesCount: function() {
    var maxColorSpace = this._blackLevel,
        realSamplesCount = this._samples,
        autoColorsPerSample = Math.max(255 - this._blackLevel, 67);

    if (this._samples === Posterizer.SAMPLES_AUTO) {
      realSamplesCount = Math.max(3, Math.round(maxColorSpace / autoColorsPerSample));
    } else if (this._samples > this._blackLevel) {
      realSamplesCount = maxColorSpace;
    }
  ...
  _putTogetherSVGDocument: function(pathTags, scale) {
    var prefix = 'pm_' + checksum(this._potrace._luminanceData.data);
    ...
    var tags = pathTags.map(function(tag, index) {
      ...
      uses.push(
        ...
        'fill-opacity="' + ((255 - tag.brightness) / 255).toFixed(3) + '" ' +
  ...
  getSVG: function(scale) {
    ...
    while (i >= 0) {
      ...
      this._potrace.setParameter({ blackLevel: threshold });
      tags.push({
        brightness: color,
