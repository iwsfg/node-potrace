...
Posterizer.prototype = {
  ...
  _calcColorIntensity: function(colorStops) {
    ...
    var histogram = colorSelectionStrat !== Posterizer.COLOR_FILL_SPREAD ? this._getImageHistogram() : null;
    ...
    return colorStops.map(function(threshold, index) {
      ...
      switch (colorSelectionStrat) {
        case Posterizer.COLOR_FILL_SPREAD:
   ...
   * Loads image.
   *
   * @param {*} target Image source
   * @param {Function} callback
   */
  loadImage: function(target, callback) {
    var self = this;

    return this._potrace.loadImage(target, function(err) {
