...
Potrace.prototype = {
  _bmToPathlist: function() {
    var self = this,
        ...
        shouldBeLower = this._params.blackOnWhite,
        blackMap,
        currentPoint = new Point(0, 0),
        path;

    if (threshold === Potrace.THRESHOLD_AUTO) {
      threshold = this._luminanceData.histogram().autoThreshold();
    }
  ...
  loadImage: function(target, callback) {
    ...
    Jimp.read(target, function(err, img) {
      ...
      if (targetHasChanged) {
        return callback.call(self, new Error('loadImage method was used with another source since last time'));
      }
