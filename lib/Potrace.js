...
Potrace.prototype = {
  _bmToPathlist: function() {
    var self = this,
        ...
        shouldBeLower = !this._params.whiteOnBlack,
        blackMap = this._luminanceData.copy(function(lum) {
          return lum < threshold === shouldBeLower ? 1 : 0;
        }),
  ...
  /**
   * Generates path tag without rest of the SVG file
   *
   * @param {Number} [scale]
   ...
   * @returns {String}
   */
  getPathTag: function(scale, useCurves, fillColor) {
    scale = scale || 1;
    useCurves = useCurves === true;
    fillColor = typeof fillColor === 'string' ? fillColor : (this._params.whiteOnBlack ? 'white' : 'black');
    ...
    if (useCurves) {
      tag += '" stroke="' + fillColor + '" fill="none"/>';
    } else {
  ...
  getSVG: function(scale, useCurves) {
    ...
    return '<svg version="1.1" ' +
      ...
      this.getPathTag(scale, useCurves) +
      '</svg>';
  }
};
 ...
 * @property {Number}  [blackLevel]   - treshold below which color is considered black (0..255, default 128)
 * @property {Boolean} [whiteOnBlack] - specifies colors by which side of blackLevel threshold should be traced (default false)
 */